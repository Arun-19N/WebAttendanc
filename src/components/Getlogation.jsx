...
const Attendance = () => {
        ...
        async (position) => {
          ...
          try {
            const res = await fetch(
              `https://maps.googleapis.com/maps/api/geocode/json?latlng=${lat},${lon}&key=AIzaSyAAHvRYJrifvG4v5TGU8KfFgVUWNMCFkZE`
            );
            const data = await res.json();

            if (data.status === 'OK' && data.results.length > 0) {
              setEmployeeAddress(data.results[0].formatted_address);
            } else {
  ...
  return (
    <section>
      <div className="admin_pannel_part d-flex justify-content-center align-items-center" style={{ height: '100vh' }}>
